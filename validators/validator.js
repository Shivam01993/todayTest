    ...
    try {
        ...
        if (model) {
            ...
            for (let key in model) {
                ...
                if (!data_source) return res.status(RESPONSE_STATUS.BAD_REQUEST).send({ "message": "Body does not have acceptable data." });
                let expected_type;
                ...
                if (model_param.required) {
                    if ((!data_source.hasOwnProperty(key)) || !data_source[key] || data_source[key] === null || data_source[key] === undefined) {
                        return res.status(RESPONSE_STATUS.BAD_REQUEST).send({ message: "Required key '" + key + "' is missing or is Invalid." });
                    }
                }
                ...
                if (data_source[key].constructor != expected_type) {
                    ...
                    try {
                        if (expected_type == Date) {
                            ...
                            input_value === "true" ? input_value = true : input_value === 'false' ? input_value = false : input_value = null;
                    ...
                    if (input_value === null || input_value === undefined) {
                        return res.status(RESPONSE_STATUS.BAD_REQUEST).send({ message: "Key '" + key + "' is '" + data_source[key].constructor.name + "'. Expected '" + expected_type.name + "'." });
                    }
                ...
                if (model_param.enum) {
                    if (model_param.enum.indexOf(data_source[key]) == -1) {
                        return res.status(RESPONSE_STATUS.BAD_REQUEST).send({ message: "Input for Key '" + key + "' is Invalid." });
                    }
                }

                if (model_param.min) {
                    if (data_source[key] < model_param.min) {
                        return res.status(RESPONSE_STATUS.BAD_REQUEST).send({ message: "Minimum value for '" + key + "' is " + model_param.min });
                    }
                }
                if (model_param.max) {
                    if (data_source[key] > model_param.min) {
                        return res.status(RESPONSE_STATUS.BAD_REQUEST).send({ message: "Maximum value for '" + key + "' is " + model_param.min });
                    }
                }
        ...
        if (error.name == "CastingError") {
            return res.status(RESPONSE_STATUS.BAD_REQUEST).send({ message: error.message });
        }
        throw error;
    }
}